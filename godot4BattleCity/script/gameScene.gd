...
var scoreLabel = preload("res://scene/score_label.tscn") # 分数显示预制体
...
var autoHideLabel = preload("res://scene/auto_hide_label.tscn") # 自动隐藏标签预制体
...
func _ready():
	...
	# 连接信号（Godot 4 使用 .connect() 方法）
	Game.baseDestroyed.connect(baseDestroyed) # 基地被摧毁信号
	Game.addBonus.connect(addBonus) # 添加道具信号
	Game.destroyEnemy.connect(destroyEnemy) # 敌人被摧毁信号
	Game.hitPlayer.connect(hitPlayer) # 玩家被击中信号
	Game.getBonus.connect(getBonus) # 获得道具信号
