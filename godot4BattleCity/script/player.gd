	...
	if canShoot: # 如果可以开火
		...
		for i in bullets: # 清理无效对象
			if not is_instance_valid(i):
				del.append(i)
		for i in del:
			bullets.remove_at(bullets.find(i)) # Godot 4 使用 .remove_at() 替代 .remove()
...
func animation(dir, vec):
	...
	if vec != Vector2.ZERO:
		if level == Game.level.MIN:
			ani.play("small_run") # 小型坦克运行动画
		elif level == Game.level.MEDIUM:
			ani.play('medium_run') # 中型坦克运行动画
		elif level == Game.level.LARGE:
			ani.play('large_run') # 大型坦克运行动画
		elif level == Game.level.SUPER:
			ani.play('super_run') # 超级坦克运行动画
	else:
		if level == Game.level.MIN:
			ani.play("small") # 小型坦克 idle动画
		elif level == Game.level.MEDIUM:
			ani.play('medium') # 中型坦克 idle动画
		elif level == Game.level.LARGE:
