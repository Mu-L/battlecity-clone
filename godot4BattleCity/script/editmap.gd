	...
	if file:
		...
		for i in currentLevel['data']:
			if int(i['type']) in [0,1,2,3,4]:  # 只加载有效的砖块类型
				var temp=tile.instantiate()  # 创建地图块
				temp.position.x=int(i['x'])*cellSize+cellSize/2+offset.x  # 设置X坐标
				temp.position.y=int(i['y'])*cellSize+cellSize/2+offset.y  # 设置Y坐标
				temp.type=int(i['type'])  # 设置砖块类型
				temp.mapPos=Vector2(int(i['x']),int(i['y']))  # 设置地图位置
				childNode.add_child(temp)  # 添加到子节点容器
	else:
...
func save2File(fileName):
	...
	var file = FileAccess.open(fileName, FileAccess.WRITE)  # 打开文件
	var json=JSON.new()
	file.store_string(json.stringify(data))  # 写入JSON数据
	file.flush()  # 刷新缓冲区
	file.close()  # 关闭文件
...
func _input(event):
	...
	if event is InputEventMouseButton:  # 鼠标按键事件
		if event.button_index == MOUSE_BUTTON_LEFT && event.pressed:  # 左键按下
			isPress=true  # 设置按下标志
